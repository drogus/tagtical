...
While a lot concepts are the same (taggings + tags tables using polymorphism), this adaption
creates subclasses on Tag which allows for users to build out Tag functionality as you wish.

For instance, if you want to tag a photo with "Color Tags". You would simply subclass Tag with
Tag::Color and you could add functionality specific to that model. This involves moving the "context"
off of Tagging and moving it onto Tag as a "type" column. It acts not only as a "context", but also
as a designator for the STI class.
...
1. Tagtical removes "context" off the taggings table and adds "type" onto the tags table.
2. custom_contexts (on the fly) contexts are removed since every tag type must map to a defined class.
3. The traditional functionality of tags is preserved while laying the foundation for STI on the Tag
...
4. Tag "names" now become tag "values". The difference is small, but significant. If you had
