  ...
  module Core
    ...
    module ClassMethods
      def initialize_tagtical_core
        tag_types.each do |tag_type|
          ...
          #  end
          scope(tag_type.scope_name, where(["#{Tagtical::Tag.table_name}.type = ? AND #{Tagtical::Tagging.table_name}.tagger_id IS NULL", tag_type]))
      ...
      def tagged_with(tags, options = {})
        ...
        if options.delete(:exclude)
          ...
          tagging_join << "  AND " + sanitize_sql(["#{Tagtical::Tag.table_name}.type = ?", tag_type(context)]) if context
          select_clause = "DISTINCT #{table_name}.*" unless context and tag_types.one?

          joins << tagging_join

        else
          tags = Tagtical::Tag.where_any(tag_list)
          return scoped(:conditions => "1 = 0") unless tags.length == tag_list.length # allow for chaining

          tags.each do |tag|
            next if context && tag.type==context
    ...
    module InstanceMethods
      ...
      def all_tags_on(context)
        ...
        scope = tags.where(["#{tag_table_name}.type = ?", tag_type(context)])
      ...
      def tags_on(context)
        tags.send(tag_type(context).scope_name).all
      end
      ...
      def save_tags
        self.class.tag_types.each do |tag_type|
          ...
          tag_list = tag_list_cache_on(tag_list).uniq
          ...
          old_taggings = taggings.find_all_by_tag_id(old_tags)
      ...
      def tag_type(input)
        (@tag_type ||= {})[context] ||= Tagtical::Tag::Type[input].tap do |tag_type|
